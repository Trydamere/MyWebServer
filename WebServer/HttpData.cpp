...
void HttpData::reset() {
    ...
    //keepAlive_ = false;
    if (timer_.lock()) {
        shared_ptr<TimerNode> my_timer(timer_.lock());//创造一个sharedptr
        my_timer->clearReq();
        timer_.reset();
    }
}
...
AnalysisState HttpData::analysisRequest() {
    if(method_ == METHOD_POST) {
        ...
        if(fileName_ == "favicon.ico") {
            ...
            header += "Server: HuangYilong's Web Server\r\n";
...
void HttpData::handleError(int fd, int err_num, string short_msg) {
    ...
    body_buff += "<hr><em> HuangYilong's Web Server</em>\n</body></html>";

    header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
    header_buff += "Content-Type: text/html\r\n";
    header_buff += "Connection: Close\r\n";
    header_buff += "Content-Lenght: " + to_string(body_buff.size()) + "\r\n";
    header_buff += "Server: HuangYilong's web Server\r\n";
    header_buff += "\r\n";
    //错误处理不考虑written不玩的情况
